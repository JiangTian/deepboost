...
vector<real> getEpsK(const Density & pw, const Dataset & S, int k); // TODO
...
Density DeepMaxent(const Dataset & S, int T) {
  ...
  for (int t = 0; t < T; ++t) {
    ...
    for (int k = 0; k < phi.size(); ++k) {
      ...
      const vector<real> epsK = getEpsK(pw, S, k);
      for (int j = 0; j < N[k]; ++j) {
	real d;
	const real wkj = w[k][j];
	const real eps = epsK[j];
    ...
    const real EphiSkj = EphiPW(phi[k], S, j);
